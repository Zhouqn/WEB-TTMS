  ...
  <div id="modify" class="row">
    ...
    <div class="modify-form col-6 col-md-6">
      <div class="form-group">
        <label for="play-id">剧目ID</label>
        <input type="text" class="form-control" id="play-id" readonly v-model="play.id">
      </div>
      ...
      <div class="" style="text-align: center;">
        <button @click="modify(play)" type="submit" class="btn btn-warning " >确定</button>
...
<script>
  ...
  export default {
    ...
    data(){
      return{
        play : Object.assign({},this.parameter),
        inputError : {
          name : false,
          duration : false,
          area : false,
    ...
    methods :{
      ...
      modify(play){
        ...
        post('play/modify',play)
          .then((response) => {
            if(response.status){
              toastr.success("剧目信息修改成功！");
